...
// ─── FAKE ORDER (review_and_pay / nativeFlowMessage) ─────────────────────────
async function sendFakeOrder(sock, jid, content = {}) {
    const {
        quoted,
        thumbnail,        // Buffer dari file gambar (opsional)
        currency    = 'USD',
        totalAmount = 100000000,
        offset      = 2,
        referenceId = `REF-${Date.now()}`,
        description = 'Fake Order by Bot',
        itemName    = 'Produk Premium',
        quantity    = '999',
        paymentNote = 'Dummy payment by Bot',
    ...
    const orderData = {
        ...
        external_payment_configurations: [{
            ...
            payment_instruction: paymentNote,
        }],
        payment_configuration: 'config',
        payment_type:          'true',
        total_amount:          { value: totalAmount, offset },
        reference_id:          referenceId,
        type:                  'physical-goods',
        order: {
            status:      'delivered',
            description,
            subtotal:    { value: totalAmount, offset: 100 },
            items: [{
                retailer_id: '60135576635',
                product_id:  '60135576635',
                name:        itemName,
                amount:      { value: totalAmount, offset },
    ...
    const headerObj = {
        hasMediaAttachment: thumbnail ? true : false,
    };
    if (thumbnail) {
        headerObj.jpegThumbnail = thumbnail;
    }
    ...
    // Bangun quoted context jika ada
    const contextInfo = buildContextInfo(quoted);
    if (contextInfo) {
        orderMessage.interactiveMessage.contextInfo = contextInfo;
    }

    return sock.relayMessage(jid, orderMessage, {
        messageId: sock.generateMessageTag(),
    });
}

module.exports = { sendButtons, sendListMessage, sendInteractiveMessage, sendButtonWithImage, sendInteractiveWithImage, sendFakeOrder };